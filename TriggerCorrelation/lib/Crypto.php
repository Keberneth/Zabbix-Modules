...
/**
 * Application-layer encryption-at-rest for the one reversible secret this module
 * stores in the Zabbix `module` config: the Zabbix API token. (The evaluation
 * shared secret is stored as a bcrypt hash and never needs to be recovered.)
 *
 * Design goals — chosen so this can never "brick" the module:
 *   - Opt-in. Encryption only activates when an encryption key is provided via
 *     the ZABBIX_TRIGGER_CORRELATION_ENCRYPTION_KEY environment variable. With
 *     no key set the module behaves exactly as before (token stored verbatim),
 *     so existing single-server installs are unaffected by upgrading.
 ...
 *     stored plaintext tokens keep working and are upgraded to ciphertext the
 *     next time settings are saved.
 *   - Fail-safe. If a value is encrypted but the key is missing/wrong (or no
 *     crypto backend is available), decryption returns '' rather than throwing —
 *     the affected call reports "token unavailable" instead of taking down the UI.
 *
 * Key handling mirrors the reference AI module: the env var value is any
 * passphrase, hashed to a fixed 32-byte key with SHA-256. Using an environment
 * variable (not a file/DB) is what makes this safe across the module's supported
 * topologies — a single server sets it once, a multi-server/split install sets
 * the SAME value on each frontend node, and Docker/Compose passes it as an
...
final class Crypto {
    ...
    private const ENV_KEY = 'ZABBIX_TRIGGER_CORRELATION_ENCRYPTION_KEY';
    ...
    /**
     * Derive the 32-byte key from the ZABBIX_TRIGGER_CORRELATION_ENCRYPTION_KEY
     * environment variable, or null when it is not set. Any passphrase is
     * accepted and hashed to a fixed-length key.
     */
