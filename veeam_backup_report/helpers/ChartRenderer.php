 ...
 *
 * Colours are emitted as `style="fill:var(--vr-s1)"` rather than literal hex so
 * one renderer serves both the light and the dark theme: the consuming page
 * defines --vr-* once per theme. Chart chrome (gridlines, axis text, baseline)
...
class ChartRenderer {
     ...
     * @param array $series      [['label' => string, 'token' => '--vr-s1', 'values' => float[]], ...]
    ...
    public function hBars(array $rows, array $opts = []): string {
        ...
        foreach ($rows as $row) {
            ...
            $svg[] = sprintf(
                '<rect class="vr-track" x="%d" y="%d" width="%d" height="%d" rx="4"/>',
                $track_x, $bar_y, $track_w, $bar_h
            );

            $svg[] = sprintf(
                '<g class="vr-mark" data-tip="%s"><title>%s</title>%s</g>',
                $tip,
                $tip,
                sprintf(
            ...
            $svg[] = sprintf(
                '<text class="vr-label" x="%d" y="%d">%s</text>',
                $label_w, $bar_y + 12, $this->text($this->truncate((string) $row['label'], 34))
            );
            $svg[] = sprintf(
                '<text class="vr-value" x="%d" y="%d">%s</text>',
    ...
    public function donut(array $slices, string $center_value, string $center_label, array $opts = []): string {
        ...
        if ($total <= 0) {
            $svg[] = sprintf(
                '<circle class="vr-track-ring" cx="%s" cy="%s" r="%s" fill="none" stroke-width="%d"/>',
            ...
            foreach ($slices as $slice) {
                ...
                $svg[] = sprintf(
                    '<g class="vr-mark" data-tip="%s"><title>%s</title><path d="%s" style="fill:var(%s)"/></g>',
                    $tip,
                    $tip,
        ...
        $svg[] = sprintf(
            '<text class="vr-donut-value" x="%s" y="%s" text-anchor="middle">%s</text>',
            $cx, $cy + 4, $this->text($center_value)
        );
        $svg[] = sprintf(
            '<text class="vr-donut-label" x="%s" y="%s" text-anchor="middle">%s</text>',
    ...
    public function lineChart(array $points, array $forecast = [], array $opts = []): string {
        ...
        if ($threshold !== null) {
            $ty = $y_at($threshold);
            $svg[] = sprintf(
                '<line class="vr-threshold" x1="%s" y1="%s" x2="%s" y2="%s"/>',
                $pad['l'], $ty, $pad['l'] + $plot_w, $ty
            );
            $svg[] = sprintf(
                '<text class="vr-threshold-label" x="%s" y="%s" text-anchor="end">%s</text>',
        ...
        if ($area !== []) {
            ...
            $area[] = 'Z';
            $svg[] = sprintf(
                '<path class="vr-area" d="%s" style="fill:var(%s)"/>',
                implode(' ', $area), self::safeToken($opts['token'] ?? null)
            );
        }

        $svg[] = sprintf(
            '<path class="vr-line" d="%s" style="stroke:var(%s)"/>',
        ...
        if ($forecast !== []) {
            ...
            $svg[] = sprintf(
                '<path class="vr-line vr-line--forecast" d="%s" style="stroke:var(%s)"/>',
        ...
        foreach ($all as $i => $point) {
            ...
            $svg[] = sprintf(
                '<g class="vr-mark" data-tip="%s"><title>%s</title><rect x="%s" y="%s" width="%s" height="%s" fill="transparent"/></g>',
        ...
        $svg[] = sprintf(
            '<circle class="vr-end-dot" cx="%s" cy="%s" r="4.5" style="fill:var(%s)"/>',
     ...
     */
    public function sparkline(array $values, string $token = '--vr-s1'): string {
        ...
        return sprintf(
            '<svg class="vr-sparkline" viewBox="0 0 %d %d" width="%d" height="%d" role="img" aria-hidden="true" focusable="false">'
    ...
    public function meter(?float $pct, array $opts = []): string {
        ...
        return sprintf(
            '<span class="vr-meter" role="img" aria-label="%s"><span class="vr-meter-fill" style="width:%s%%;background:var(%s)"></span></span>',
    ...
    public function legend(array $items): string {
        ...
        $out = ['<div class="vr-legend">'];
        foreach ($items as $item) {
            $out[] = sprintf(
                '<span class="vr-legend-item"><span class="vr-legend-swatch" style="background:var(%s)"></span>'
                    .'<span class="vr-legend-label">%s</span>%s</span>',
                self::safeToken($item['token'] ?? null),
                $this->text((string) $item['label']),
                isset($item['text']) && $item['text'] !== ''
                    ? '<span class="vr-legend-value">'.$this->text((string) $item['text']).'</span>'
                    : ''
            );
        }
        $out[] = '</div>';
    ...
    private function stackedArea(array $categories, array $series, array $opts): string {
        ...
        foreach ($series as $s) {
            ...
            $svg[] = sprintf(
                '<path class="vr-stack-area" d="%s" style="fill:var(%s)"><title>%s</title></path>',
                implode(' ', $path),
                self::safeToken($s['token'] ?? null),
                $this->text((string) $s['label'])
            );
    ...
    private function yAxis(array $scale, array $pad, float $plot_w, float $plot_h, callable $format): string {
        ...
        foreach ($scale['ticks'] as $tick) {
            ...
            $out[] = sprintf(
                ...
                $tick > 0 ? 'vr-grid' : 'vr-baseline',
                $pad['l'], round($y, 1), $pad['l'] + $plot_w, round($y, 1)
            );
            $out[] = sprintf(
                '<text class="vr-tick" x="%s" y="%s" text-anchor="end">%s</text>',
    ...
    private function xAxis(array $labels, array $pad, float $plot_w, float $plot_h, bool $banded = true): string {
        ...
        foreach ($labels as $i => $label) {
            ...
            $out[] = sprintf(
                '<text class="vr-tick" x="%s" y="%s" text-anchor="middle">%s</text>',
    ...
    private function openSvg(int $width, int $height, string $title): string {
        return sprintf(
            '<svg class="vr-chart" viewBox="0 0 %d %d" preserveAspectRatio="xMidYMid meet" role="img" aria-label="%s" focusable="false">',
            $width, $height, $this->text($title)
        );
    }

    private function emptyChart(string $message): string {
        return '<p class="vr-chart-empty">'.$this->text($message).'</p>';
    }
