...
class PromptBuilder {
    ...
    public static function buildActionsSystemPrompt(array $config, array $permissions): string {
        ...
        $blocks[] = '- CRITICAL pitfall #1 (uniqueness): a generic tag shared across environments/instances blends them. If dev, test and prod all emit service=filezilla, a service scoped only on service=filezilla combines all three — wrong. Scope to a UNIQUE identifier.';
        $blocks[] = '- CRITICAL pitfall #2 (granularity — host vs service): decide whether the operator wants HOST availability (the server is up/reachable) or a SPECIFIC SERVICE on the host (e.g. the MSSQL database engine, an FTP service, a web app). These measure different things. A HOST-level tag is inherited by EVERY problem on the host, so it measures "any problem on the host", not one service — if you use it for a service SLA, an unrelated CPU/disk problem counts as the service being down and the SLA lies. For a SERVICE SLA you MUST scope to that service\'s OWN availability trigger(s), not the host.';
        $blocks[] = '- Required workflow when the operator asks to create/set up an SLA:';
        $blocks[] = '  1. ASK the operator (unless already explicit): should this measure HOST availability (the server reachable) or a SPECIFIC SERVICE running on it? Plus the scope (which host/environment). Do not silently assume host-level.';
        ...
        $blocks[] = '  2. Call analyze_sla_scope (hostnames or group_name, plus the service keyword) to see the REAL host/template/trigger tags and whether a unique tag or AND-combination already exists. Never assume a tag is unique.';
        $blocks[] = '  3. Choose the unique matcher: a single unique tag, or several existing tags AND-combined in the service problem_tags (e.g. service=filezilla AND env=prod). Prefer existing tags.';
        ...
        $blocks[] = '  5. create_sla_service with the AND-combined problem_tags (the unique matcher) and a unique service tag.';
        $blocks[] = '  6. create_sla with service_tags equal to that unique service tag.';
