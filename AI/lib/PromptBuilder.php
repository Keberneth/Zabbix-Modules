...
class PromptBuilder {
    ...
    public static function buildActionsSystemPrompt(array $config, array $permissions): string {
        ...
        $blocks[] = '  · several hosts as ONE grouped SLA (including HA/failover clusters) → one leaf service per host, then a parent service with child_serviceids of those leaves (NO problem_tags — Zabbix forbids a service having both problem_tags and children) tagged sla_scope=<app>.<env>.cluster; the SLA selects the PARENT\'s tag only. The parent\'s algorithm MUST match the topology: 1 = "most critical if ALL children have problems" for a redundant HA cluster (the service stays UP while at least one node is up — a single-node failover is NOT downtime), 2 = "most critical of child services" when every node matters (any node down counts as downtime).';
        ...
        $blocks[] = '  1c. ALWAYS ask whether the service runs on MULTIPLE hosts as a high-availability/failover/cluster setup — even when the operator names only one host, a standby node may exist. If it is HA/clustered: build one leaf service per cluster host under a parent service (per the grouped structure above) and ask which semantics apply — is the service still UP when one node is down? YES (redundant) → parent algorithm 1; NO (every node must run) → parent algorithm 2. The SLA selects the PARENT\'s sla_scope tag. Choosing algorithm 2 for a redundant HA pair records every routine failover as an SLA breach.';
        ...
        $blocks[] = '  5. create_sla_service for each LEAF (AND-combined problem_tags + a unique sla_scope service tag), then any PARENT/GROUP services (child_serviceids instead of problem_tags; algorithm is REQUIRED for parents — 1 for redundant HA/failover clusters, 2 when any child down counts, per the operator\'s answer in step 1c), matching the structure for the confirmed scope level.';
        ...
        $blocks[] = '- The pre-write confirmation for create_sla MUST state: SLA name, SLO, period, the sla_scope tag used, the EXACT service(s) that tag matches (by name, from get_services), the problem_tags on the underlying service(s), and any parent/child services being created. When a parent/group service is involved, the confirmation MUST also state its status rule and meaning (algorithm 1 = redundant HA, down only when ALL children are down; algorithm 2 = any child down counts) with a one-line justification.';
        $blocks[] = '- Backend guardrails REJECT unsafe calls: create_sla with multiple service_tags, operator=contains, broad tag names (service/env/host/scope/application/…), tags matching more than one service, tags matching ZERO services (always rejected — create the target service first; no flag bypasses it), tags matching service(s) whose status rule is "set status to OK" (algorithm 0 — their SLI is pinned at 100%), names over 255 characters, an invalid timezone, or an effective_date not formatted YYYY-MM-DD; create_sla_service with a duplicate name, both problem_tags and child_serviceids (Zabbix forbids that combination), a missing sla_scope tag, more than one sla_scope entry (exactly ONE per service), a non-unique sla_scope value, problem_tags built ONLY from broad category tags (service/env/scope/… with no host-identifying or dedicated unique tag), a parent/group service without an EXPLICIT algorithm choice (1 or 2), or algorithm 0. The bypass flags (allow_multiple_matching_services, allow_shared_service_tag, allow_broad_problem_tags) may ONLY be set after the operator explicitly confirmed that exact broad behaviour in chat — never set them on your own initiative.';
